...
;(function (scope) {
    ...
    */
    function clippedCircleXRange(center, radius, y1, y2) {
        if (center.y >= y1 && center.y <= y2) return { x1: center.x - radius, x2: center.x + radius };
    ...
    function adaptiveOffsetFnGenerator(getTopCorner, getBottomCorner, getBoxOffset, getCornerOffset) {
        return function (y1, y2, areaLimit) {
            // y2 >= y1
            if (!this.rect.overlapsYRange(y1, y2)) return [{ x: undefined, height: y2 - y1 }];

            var offsets = [];

            if (y1 < this.rect.y) offsets.push({ x: undefined, height: this.rect.y - y1 });
            ...
            if (boxArea.overlapsYRange(y1, y2)) offsets.push({ x: getBoxOffset(boxArea), height: maxY - minY });
            ...
            if (y2 > this.rect.maxY) offsets.push({ x: undefined, height: y2 - this.rect.maxY });
    ...
    /*{mode: "step"}*/function fakeIt(element, offsets) {
    ...
    Polyfill.prototype.polyfill = function (element, settings) {
        ...
        if (!(/left|right/.test(computedStyle.cssFloat) && element.getAttribute('data-shape-outside'))) return;
    ...
    function positionOffsetToPixels(offset, extent, metrics) {
        ...
        switch (offset[0]) {
            ...
            case 'bottom':case 'right':
                direction = 'BottomRight';break;
            case 'center':
                length = extent / 2.0;break;
            default:
    ...
    ShapeValue.prototype.parseEllipsoid = function (args) {
        ...
        if (args && args[2]) {
            ...
            positions.forEach(function (position) {
                // if it is an offset
                if (/\d+/.test(position) && canMergeBack) resolvedPositions[resolvedPositions.length - 1] += ' ' + position;else resolvedPositions.push(position);
...
//# sourceMappingURL=shapes-polyfill.js.map
